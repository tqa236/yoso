...
def get_arguments():
    ...
    parser.add_argument(
        "--save-results", action="store_true", help="whether to save the results."
    )
    parser.add_argument(
        "--flip", action="store_true", help="random flip during the test."
    )
    parser.add_argument(
        "--multi-scales", type=str, default="1", help="multiple scales during the test"
    )
...
def main():
    ...
    model = networks.init_model(
        args.arch, num_classes=args.num_classes, pretrained=None
    )
    ...
    # Data loader
    lip_test_dataset = CropDataValSet(
        args.data_dir,
        args.split_name,
    ...
    testloader = data.DataLoader(
        lip_test_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True
    )
