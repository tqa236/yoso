...
class IPAttnProcessor_clothpass_noip(torch.nn.Module):
    ...
    def __call__(
        ...
        if is_cloth_pass or up_cnt is None:
            ...
            cloth_feature = rearrange(
                cloth_feature, "b c h w -> b (h w) c"
            ).contiguous()
            ...
            hidden_states_cloth = hidden_states_cloth.transpose(1, 2).reshape(
                batch_size, -1, attn.heads * head_dim
            )
...
class IPAttnProcessor2_0_Lora(torch.nn.Module):
    ...
    def __init__(
        ...
        self.to_k_ip_lora = LoRALinearLayer(
            in_features=self.to_k_ip.in_features,
            out_features=self.to_k_ip.out_features,
            rank=rank,
        )
        self.to_v_ip_lora = LoRALinearLayer(
            in_features=self.to_v_ip.in_features,
            out_features=self.to_v_ip.out_features,
            rank=rank,
        )
