...
def bbox_expand(img_height, img_width, bbox, exp_ratio):
    ...
    new_x_max = (
        img_width - 1 if x_max + exp_x > img_width - 1 else np.round(x_max + exp_x)
    )
    new_y_max = (
        img_height - 1 if y_max + exp_y > img_height - 1 else np.round(y_max + exp_y)
    )
...
def make_crop_and_mask(img_info, pred, file_list, crop_save_dir, mask_save_dir, args):
    ...
    for instance in pred[img_id]["instances"]:
        ...
        bbox_name = (
            os.path.splitext(img_name)[0] + "_" + str(person_idx) + "_msrcnn.jpg"
        )
...
def main(args):
    ...
    for img_info in tqdm(img_info_list["images"]):
        json_file, file_list = make_crop_and_mask(
