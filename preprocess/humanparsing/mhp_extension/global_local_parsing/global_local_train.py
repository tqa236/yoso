...
def get_arguments():
    ...
    parser.add_argument(
        "--cycle-epochs", type=int, default=10, help="schp cyclical epoch"
    )
    parser.add_argument(
        "--schp-restore", type=str, default="./log/schp_checkpoint.pth.tar"
    )
    parser.add_argument(
        "--lambda-s", type=float, default=1, help="segmentation loss weight"
    )
...
def main():
    ...
    train_dataset = LIPDataSet(
        args.data_dir, args.split_name, crop_size=input_size, transform=transform
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i_iter, batch in enumerate(train_loader):
            ...
            if i_iter % 100 == 0:
                print(
                    "iter = {} of {} completed, lr = {}, loss = {}".format(
                        i_iter, total_iters, lr, loss.data.cpu().numpy()
                    )
                )
        ...
        print(
            "epoch = {} of {} completed using {} s".format(
