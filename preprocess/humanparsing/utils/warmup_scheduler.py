...
class GradualWarmupScheduler(_LRScheduler):
    ...
    def __init__(
        self, optimizer, total_epoch, eta_min=0, warmup_epoch=10, last_epoch=-1
    ):
    ...
    def get_lr(self):
        if self.last_epoch <= self.warmup_epoch:
            return [
                self.eta_min
                + self.last_epoch * (base_lr - self.eta_min) / self.warmup_epoch
...
class SGDRScheduler(_LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_epoch:
            ...
            return [
                ...
                * (
                    ...
                    + math.cos(
                        math.pi
                        * (self.last_epoch - self.warmup_epoch)
                        / (self.start_cyclical - self.warmup_epoch)
                    )
                )
                / 2
                for base_lr in self.base_lrs
            ...
            return [
                self.eta_min
                + (self.cyclical_base_lr - self.eta_min)
                * (
                    1
                    + math.cos(
                        math.pi
                        * (
                            (self.last_epoch - self.start_cyclical)
                            % self.cyclical_epoch
                        )
                        / self.cyclical_epoch
                    )
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.SGD(
        params=model.parameters(), lr=7e-3, momentum=0.9, weight_decay=5e-4
    )
    scheduler_warmup = SGDRScheduler(
        optimizer,
        total_epoch=150,
        eta_min=7e-5,
        warmup_epoch=10,
        start_cyclical=100,
        cyclical_base_lr=3.5e-3,
        cyclical_epoch=10,
    )
    lr = []
