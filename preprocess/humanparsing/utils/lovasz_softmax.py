...
def lovasz_softmax(
    probas, labels, classes="present", per_image=False, ignore=255, weighted=None
):
    """
    ...
    if per_image:
        ...
        loss = lovasz_softmax_flat(
            *flatten_probas(probas, labels, ignore), classes=classes, weighted=weighted
        )
    return loss
...
def lovasz_softmax_flat(probas, labels, classes="present", weighted=None):
    ...
    for c in class_to_sum:
        ...
        if weighted is not None:
            losses.append(
                weighted[c] * torch.dot(errors_sorted, Variable(lovasz_grad(fg_sorted)))
            )
        else:
